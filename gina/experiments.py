...
class LC_model(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            LocallyConnected1D(field_size=5, outputs_per_field=15, stride=5),
            nn.ReLU(),
            LocallyConnected1D(field_size=30, outputs_per_field=15, stride=30), 
            nn.ReLU(), 
            LocallyConnected1D(field_size=30, outputs_per_field=6, stride=30),
            nn.ReLU(),
            LocallyConnected1D(field_size=294, outputs_per_field=50, stride=294),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=50, outputs_per_field=294, stride=50),
            nn.ReLU(),
            LocallyConnected1D(field_size=6, outputs_per_field=30, stride=6),
            nn.ReLU(),
            LocallyConnected1D(field_size=15, outputs_per_field=30, stride=15),
            nn.ReLU(), 
            LocallyConnected1D(field_size=15, outputs_per_field=5, stride=15),
        )
...
class dense_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(in_features=970, out_features=92),
            nn.ReLU(),
            nn.Linear(in_features=92, out_features=10)  
        )
        self.decoder = nn.Sequential(
            nn.Linear(in_features=10, out_features=92), 
            nn.ReLU(), 
            nn.Linear(in_features=92, out_features=970)
        )
