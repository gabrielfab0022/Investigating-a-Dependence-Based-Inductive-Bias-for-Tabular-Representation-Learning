...
class LC_model(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            LocallyConnected1D(field_size=20, outputs_per_field=40, stride=20),
            nn.ReLU(),
            LocallyConnected1D(field_size=80, outputs_per_field=10, stride=80), 
            nn.ReLU(), 
            LocallyConnected1D(field_size=380, outputs_per_field=50, stride=380),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=50, outputs_per_field=380, stride=50),
            nn.ReLU(),
            LocallyConnected1D(field_size=10, outputs_per_field=80, stride=10),
            nn.ReLU(),
            LocallyConnected1D(field_size=40, outputs_per_field=20, stride=40),
        )
...
class dense_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(in_features=1503, out_features=76),
            nn.ReLU(),
            nn.Linear(in_features=76, out_features=10)  
        )
        self.decoder = nn.Sequential(
            nn.Linear(in_features=10, out_features=76), 
            nn.ReLU(), 
            nn.Linear(in_features=76, out_features=1503)
        )
