...
class LC_model(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=40, outputs_per_field=10, stride=40), 
            nn.ReLU(), 
            LocallyConnected1D(field_size=150, outputs_per_field=50, stride=150),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=50, outputs_per_field=150, stride=50),
            nn.ReLU(),
            LocallyConnected1D(field_size=10, outputs_per_field=40, stride=10),
            nn.ReLU(), 
...
class dense_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(in_features=294, out_features=66),
            nn.ReLU(),
            nn.Linear(in_features=66, out_features=10)  
        )
        self.decoder = nn.Sequential(
            nn.Linear(in_features=10, out_features=66), 
            nn.ReLU(), 
            nn.Linear(in_features=66, out_features=294)
        )
