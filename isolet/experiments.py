...
class LC_model(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=60, outputs_per_field=10, stride=60),
            nn.ReLU(),
            LocallyConnected1D(field_size=210, outputs_per_field=50, stride=210),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            LocallyConnected1D(field_size=50, outputs_per_field=210, stride=50),
            nn.ReLU(),
            LocallyConnected1D(field_size=10, outputs_per_field=60, stride=10),
            nn.ReLU(),
...
class dense_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(in_features=617, out_features=70),
            nn.ReLU(),
            nn.Linear(in_features=70, out_features=10)  
        )
        self.decoder = nn.Sequential(
            nn.Linear(in_features=10, out_features=70), 
            nn.ReLU(), 
            nn.Linear(in_features=70, out_features=617)
        )
